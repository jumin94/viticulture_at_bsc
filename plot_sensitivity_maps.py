...
def plot_sensitivity_pr_viejo(indices,indices_pval,frac_var,path_fig):
    ...
    fig = plt.figure(figsize=(20, 16),dpi=100,constrained_layout=True)
    ...
    ax4 = plt.subplot(2,3,4,projection=projection)
    ...
    ax5 = plt.subplot(2,3,5,projection=projection)
    ...
    plt.title('e) Global warming',fontsize=18)
    ...
    ax6 = plt.subplot(2,3,6,projection=projection)
    ...
    plt.title('f) Fraction of variance explained',fontsize=18)
    ...
    colorbar_axes4 = fig.add_axes([fourth_plot_left +0.23, bottom_6+0.1, 0.01, height_4*0.6])
    ...
    colorbar_axes5 = fig.add_axes([fifth_plot_left +0.23, bottom_6+0.1, 0.01, height_5*0.6])
    ...
    colorbar_axes1 = fig.add_axes([first_plot_left +0.23, bottom_1, 0.01, height_1*0.6])
    cbar = fig.colorbar(im1, colorbar_axes1, orientation='vertical')
    cbar.set_label('mm day$^{-1}$K$^{-1}$',fontsize=14) #rotation = radianes
    cbar.ax.tick_params(axis='both',labelsize=14)

    second_plot_left = plt2_ax.get_position().bounds[0]
    colorbar_axes2 = fig.add_axes([second_plot_left +0.23, bottom_2, 0.01, height_2*0.6])
    cbar = fig.colorbar(im2, colorbar_axes2, orientation='vertical')
    cbar.set_label('mm day$^{-1}$K$^{-1}$',fontsize=14) #rotation = radianes
    cbar.ax.tick_params(axis='both',labelsize=14)

    third_plot_left = plt3_ax.get_position().bounds[0]
    colorbar_axes3 = fig.add_axes([third_plot_left +0.23, bottom_3, 0.01, height_3*0.6])
    ...
    colorbar_axes6 = fig.add_axes([sixth_plot_left +0.23, bottom_6+0.1, 0.01, height_6*0.6])
...
def plot_sensitivity_pr(indices,indices_pval,frac_var,path_fig,title):
...
def plot_sensitivity_ua(indices,indices_pval,frac_var,path_fig,title,season,full_season='yes'):
...
def plot_sensitivity_ua_extratropical(indices,indices_pval,frac_var,path_fig,title,season,full_season='yes'):
    ...
    ax6.add_feature(cartopy.feature.BORDERS, linestyle='-', alpha=.5)
    ax6.gridlines(crs=data_crs, linewidth=0.3, linestyle='-')
    ax6.set_extent([-180, 180, -90, -25], ccrs.PlateCarree())
    plt6_ax = plt.gca()
    left_6, bottom_6, width_6, height_6 = plt6_ax.get_position().bounds
...
def plot_sensitivity_psl(indices,indices_pval,frac_var,path_fig,title):
...
def plot_sensitivity_zg(indices,indices_pval,frac_var,path_fig,title):
...
def plot_sensitivity_tas(indices,indices_pval,frac_var,path_fig,title):
    ...
    cmap = mpl.colors.ListedColormap(['darkmagenta','darkviolet','magenta','deeppink','blue','royalblue','lightskyblue','lightcyan','white','white','lemonchiffon','navajowhite','burlywood','sandybrown','coral','darkorange','tomato','orangered'])
    cmap.set_over('red')
    ...
    ax2.set_boundary(circle, transform=ax2.transAxes)
    #clevels = np.arange(-.08,.1, 0.02)
    im2=ax2.contourf(lon_c, lat, vb_gw,clevels,transform=data_crs,cmap=cmap,extend='both')
    levels = [vb_gwp.min(),0.05,1]
    ax2.contourf(lon_c, lat,vb_gwp,levels, transform=data_crs,levels=levels, hatches=["...", ""], alpha=0)
    plt.title('b) Stratospheric polar vortex',fontsize=18)
    ax2.add_feature(cartopy.feature.COASTLINE,alpha=.5)
    ax2.add_feature(cartopy.feature.BORDERS, linestyle='-', alpha=.5)
    ...
    ax3.set_boundary(circle, transform=ax3.transAxes)
    #clevels = np.arange(-.08,.1, 0.02)
    im3=ax3.contourf(lon_c, lat, sst1,clevels,transform=data_crs,cmap=cmap,extend='both')
    levels = [sst1p.min(),0.05,1]
    ax3.contourf(lon_c, lat,sst1p,levels, transform=data_crs,levels=levels, hatches=["...", ""], alpha=0)
    plt.title('c) Sea Surface Temperature (Central)',fontsize=18)
    ax3.add_feature(cartopy.feature.COASTLINE,alpha=.5)
    ax3.add_feature(cartopy.feature.BORDERS, linestyle='-', alpha=.5)
    ...
    ax4.set_boundary(circle, transform=ax4.transAxes)
    #clevels = np.arange(-.08,.1, 0.02)
    im4=ax4.contourf(lon_c, lat, sst2,clevels,transform=data_crs,cmap=cmap,extend='both')
    levels = [sst2p.min(),0.05,1]
    ax4.contourf(lon_c, lat,sst2p,levels, transform=data_crs,levels=levels, hatches=["...", ""], alpha=0)
    plt.title('d) Sea Surface Temperature (Eastern)',fontsize=18)
    ax4.add_feature(cartopy.feature.COASTLINE,alpha=.5)
    ax4.add_feature(cartopy.feature.BORDERS, linestyle='-', alpha=.5)
    ...
    circle = mpath.Path(verts * radius + center)
    ax5.set_boundary(circle, transform=ax5.transAxes)
    clevels = np.arange(-1.5,2,0.5)
    im5=ax5.contourf(lon_c, lat, gw,clevels,transform=data_crs,cmap=cmap,extend='both')
    levels = [gwp.min(),0.05,1]
    ax5.contourf(lon_c, lat, gwp,levels, transform=data_crs,levels=levels, hatches=["...", ""], alpha=0)
    plt.title('e) Global warming',fontsize=18)
    ax5.add_feature(cartopy.feature.COASTLINE,alpha=.5)
    ax5.add_feature(cartopy.feature.BORDERS, linestyle='-', alpha=.5)
...
def plot_sensitivity_ua_carree(indices,indices_pval,frac_var,path_fig,title,season,full_season='yes'):
    ...
    u_ERA = xr.open_dataset(path_era+'/era5.mon.mean.nc')
    if full_season == 'yes':
        u_ERA = u_ERA.u.sel(lev=850).h.groupby('time.season').sel(time=slice('1979','2010')).mean(dim='time')
        u_ERA = u_ERA.sel(season=season).isel(season=0)
    else:
        u_ERA = u_ERA.u.sel(lev=850).sel(time=slice('1979','2010')).sel(time=cross_year_season(u_ERA.sel(time=slice('1979','2010'))['time.month'],season)).mean(dim='time')
    ...
    ax6 = plt.subplot(3,2,5,projection=projection)
    ax6.set_extent([0,359.9, -90, 0], crs=data_crs)
    clevels = np.arange(-.4,.45,.15)
    ...
    ax7 = plt.subplot(3,2,6,projection=projection)
    ...
    colorbar_axes7 = fig.add_axes([left_7 +0.25, bottom_7-0.1, 0.01, height_7*1])
...
def plot_sensitivity_zg_carree(indices,indices_pval,frac_var,path_fig,title):
...
def plot_sensitivity_tas_carree(indices,indices_pval,frac_var,path_fig,title,extent=[-180, 180, -90, 0]):
    ...
    ax6 = plt.subplot(3,2,5,projection=projection)
    ax6.set_extent([0,359.9, -90, 0], crs=data_crs)
    clevels = np.arange(-1,1.5,.5)
    im6=ax6.contourf(lon_c, lat, gw,clevels,transform=data_crs,cmap=cmap,extend='both')
    levels = [gwp.min(),0.05,1]
    ax6.contourf(lon_c, lat, gwp,levels, transform=data_crs,levels=levels, hatches=["...", ""], alpha=0)
    plt.title('e) Global warming',fontsize=18)
    ax6.add_feature(cartopy.feature.COASTLINE,alpha=.5)
    ...
    ax7.add_feature(cartopy.feature.BORDERS, linestyle='-', alpha=.5)
    ax7.gridlines(crs=data_crs, linewidth=0.3, linestyle='-')
    ax7.set_extent(extent, ccrs.PlateCarree())
    plt7_ax = plt.gca()
    left_7, bottom_7, width_7, height_7 = plt7_ax.get_position().bounds
...
def plot_sensitivity_pr_carree(indices,indices_pval,frac_var,path_fig,title,extent=[-180, 180, -90, 0]):
    ...
    ax6.set_extent([0,359.9, -90, 0], crs=data_crs)
    clevels = np.arange(-.3,.36,.06)
    im6=ax6.contourf(lon_c, lat, gw,clevels,transform=data_crs,cmap=cmapPr,extend='both')
    levels = [gwp.min(),0.05,gwp.max()]
    #ax6.contourf(lon_c, lat, gwp,levels, transform=data_crs,levels=levels, hatches=["...", ""], alpha=0)
    plt.title('e) Global warming',fontsize=18)
    ax6.add_feature(cartopy.feature.COASTLINE,alpha=.5)
    ...
    ax7 = plt.subplot(3,2,6,projection=projection)
    ax7.set_extent(extent, crs=data_crs)
    clevels = np.arange(0,1,.1)
    im7=ax7.contourf(lon_c, latr, fv,clevels,transform=data_crs,cmap='OrRd',extend='both')
    plt.title('f) Fraction of variance explained',fontsize=18)
    ax7.add_feature(cartopy.feature.COASTLINE,alpha=.5)
